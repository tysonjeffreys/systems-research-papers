...
\subsubsection{5.1 Reflection without thrash alignment}
...
The governor interpretation is additive and testable: require evidence delta for durable rewrites, cap revise/retry cycles, and terminate with converge/abstain/escalate rather than unbounded synthesis. Companion hub note: \emph{Reflection Without Thrash} (\texttt{papers/reflection-without-thrash/}).
...
\noindent\textbf{Scope closure and basis-change.}
Within the base model scope (distribution $\rightarrow$ decoding $\rightarrow$ constraints), apparent self-knowledge is not a reliable primitive: critics and self-reports must be treated as sensors, not authorities.
The basis changes when the system can persist or act (memory writes, side-effectful tools, policy updates, delegated execution).
In that regime, abstention/tie telemetry is not optional---it is the control surface that prevents verifier-free cascades by withholding commit rights until discriminating evidence exists.

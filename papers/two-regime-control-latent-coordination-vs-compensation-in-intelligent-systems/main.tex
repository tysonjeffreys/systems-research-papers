...
\hypertarget{reflection-without-thrash-governor}{%
\subsubsection{5.6 Reflection without thrash: planning-layer governor}\label{reflection-without-thrash-governor}}
...
A minimal planning-layer governor is additive: require evidence-delta for durable commitment rewrites, cap revise/retry loops per phase, and force a terminal outcome (converge, abstain, or escalate) when the budget is exhausted. See the companion hub note \emph{Reflection Without Thrash} (\texttt{papers/reflection-without-thrash/}) for the RSML failure signatures and stress-test structure.

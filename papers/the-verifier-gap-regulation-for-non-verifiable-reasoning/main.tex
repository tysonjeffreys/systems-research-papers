...
\noindent\textbf{Scope closure and basis-change triggers.}
Within the base model scope (distribution $\rightarrow$ decoding $\rightarrow$ constraint transforms), the system may produce fluent self-descriptions of goals, beliefs, or certainty, but these are not verifiers and must not be treated as authorities.
The basis changes when we introduce persistent state or execution authority (memory writes, side-effectful tools, policy updates, delegated action).
In that regime, verifier-free failure shifts from ``wrong answer'' to \emph{compounding cascades}---and commit gating becomes the primary safety question: \emph{when is a durable action permitted?}

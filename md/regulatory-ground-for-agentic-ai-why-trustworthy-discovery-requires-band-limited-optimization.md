...
**Version:** v1.4.1  
**PDF:** ../v1.4.1_Regulatory%20Ground%20for%20Agentic%20AI%20%7C%20Why%20Trustworthy%20Discovery%20Requires%20Band-Limited%20Optimization/v1.4.1_Regulatory%20Ground%20for%20Agentic%20AI%20%7C%20Why%20Trustworthy%20Discovery%20Requires%20Band-Limited%20Optimization.pdf  
**Source:** ../v1.4.1_Regulatory%20Ground%20for%20Agentic%20AI%20%7C%20Why%20Trustworthy%20Discovery%20Requires%20Band-Limited%20Optimization/  
**Changelog:** (not found)
...
- Safe RL / CMDPs: learn while satisfying constraints [@kushwaha2025saferl_cmdp_survey; @ni2025safe_exploration_cmdp].

- Runtime assurance (RTA) / shielding: enforce a safe set via switching or filtering [@miller2024rta_rl_iccps].

- Agentic safety evaluation: trace-level risk discovery and mitigation for real-world workflows [@ghosh2025agentic].
...
Discovery requires stepping into unknown territory; without constraints, exploration repeatedly crosses unsafe boundaries before learning to avoid them (or never learns). SafeRL addresses this in the formal CMDP setting [@kushwaha2025saferl_cmdp_survey; @ni2025safe_exploration_cmdp].
...
The central design pattern is separation of competence from safety: treat the agent policy as an untrusted controller, and enforce safety through an independent regulator layer (analogous to RTA switching between an untrusted controller and a safety controller [@miller2024rta_rl_iccps]).
...
- P1: Separate competence from safety. Safety is enforced even when competence fails [@miller2024rta_rl_iccps].
...
For each candidate action $a$, compute a risk score: $$\begin{equation}
...
In embodied control, biological regulation uses global constraint signals---variables that affect all subsystems and therefore synchronize distributed control without issuing explicit commands. These signals function as boundary conditions within which local controllers operate [@jeffreys2025baseline_constraint]. The same pattern can be instantiated in agent stacks as a global restraint signal (GRS): a slow, shared scalar (or low-dimensional vector) that every component respects as a set of shared budgets [@jeffreys2025patterns_agent_llm]. Let $g(t)\in[0,1]$ denote the global restraint state, where higher values imply tighter operating posture. A minimal deterministic definition is: $$\begin{equation}
...
- **Govern the critic like the mapping.** Critics drift and can be gamed; version the judge model/prompt and monitor calibration and tie-rate drift, with explicit rollback criteria [@cai2025escapingverifier].

The regulator computes effective budgets and thresholds as functions of $g(t)$, for example: $$\begin{align}
b_{\mathrm{eff}}(t) &= b_{\min} + (1-g(t))(b_0-b_{\min})\\
\tau_{\mathrm{Yellow}}(t) &= \tau_{\mathrm{Yellow},0} - k\,g(t)
\end{align}$$ Thus, the system tightens exploration automatically when telemetry indicates rising constraint pressure. Importantly, $g(t)$ does not directly select actions; it biases the permissible operating region, keeping planner, router, and executor synchronized rather than letting each subsystem compensate locally (a common source of thrash).
...
- and downgrade capabilities (e.g., disable external WRITE/EXEC) until stabilization signals recover. Operationalization. Let $g(t)$ be the global restraint signal (Section 7). Define an activation/load proxy $L(t)$ (e.g., anomaly rate, rollback count, near-miss frequency, uncertainty spikes). Enforce duty-cycle bounds such as: $$\begin{align}
  ...
  \end{align}$$ and require a recovery procedure when exceeded.
...
- Artifact persistence: does the system store a reusable causal artifact (e.g., a "concept container") that reduces future recomputation and reduces reversal count? [@jeffreys2026concept_containers; @jeffreys2026time_to_analysis]
...
We adopt a runtime assurance (RTA) pattern: an untrusted controller proposes actions, while a safety controller (or action filter) enforces the safe set. When safe set is threatened, switch to safety controller. This pattern is formalized and studied in RTA literature [@miller2024rta_rl_iccps].
...
Tasks require: hypothesis generation, evidence gathering, belief revision, and a replicable artifact. Score includes constraints + reproducibility + calibration. Trace-level evaluation and risk discovery align with frameworks for real-world agentic workflows [@ghosh2025agentic].

## Benchmark family B: constrained learning (SafeRL)

Compare unregulated vs regulated agents in CMDPs: constraint satisfaction during learning, constrained regret, and tail-risk outcomes [@kushwaha2025saferl_cmdp_survey; @ni2025safe_exploration_cmdp].
...
This paper is one part of a broader Baseline research program on regulation as an architectural primitive. Companion work develops: (i) baseline regulation and global constraint signals for embodied control [@jeffreys2025baseline_constraint]; (ii) a two-regime lens (latent coordination vs. compensation) with a measurable compensation index and slow regulator [@jeffreys2026two_regime_control]; (iii) baseline-aware energy posture for intelligent systems [@jeffreys2026waste_energy]; (iv) architectural design patterns for baseline regulation in agent stacks and embodied systems [@jeffreys2025patterns_agent_llm; @jeffreys2025patterns_embodied]; and (v) representation- and workflow-level regulation via concept containers and timeto-analysis layers [@jeffreys2026concept_containers; @jeffreys2026time_to_analysis]. Human-side baseline and metabolic regulation are treated separately [@jeffreys2026coherent_human; @jeffreys2026lower_metabolic].
